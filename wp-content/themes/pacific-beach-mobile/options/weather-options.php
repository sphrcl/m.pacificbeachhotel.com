...
$meta_box_press = array(
	...
	'fields' => array(
		...
		array( 
              "name" => "Put this alert live",
	          "id" => $prefix."_weather_live",
	          "type" => "checkbox",
	          "std" => ""
              )	
        ,		
...
add_action('admin_menu', 'weather_press_add_box');
// Add meta box
function weather_press_add_box() {
	global $meta_box_press;
	foreach ( array( 'weather' ) as $page )
	add_meta_box($meta_box_press['id'], $meta_box_press['title'], 'weather_press_show_box', $page, $meta_box_press['context'], 			$meta_box_press['priority']);
}
// Callback function to show fields in meta box
function weather_press_show_box() {
...
jQuery(document).ready(function() {
	...
	echo '<input type="hidden" name="weather_press_meta_box_press_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
...
add_action('save_post', 'weather_press_save_data');
// Save data from meta box
function weather_press_save_data($post_id) {
	global $meta_box_press;	
	// verify nonce
	if (!wp_verify_nonce($_POST['weather_press_meta_box_press_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	// check autosave
