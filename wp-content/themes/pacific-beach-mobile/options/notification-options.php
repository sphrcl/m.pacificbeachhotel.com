...
$meta_boxel = array(
	'id' => 'CUSTOM FIELDS',
	'title' => 'Additional Options for This Notification',
	...
	'fields' => array(
		...
		array( 
			"name" => "Link",
			"desc" => "Paste in the full link to book this special: http://example.com",
			"id" => $prefix."_special",
			"type" => "text",
			"std" => ""
		),
		...
		array( 
			"name" => "Icon",
			"desc" => "Please refer to http://fontawesome.io/icons/ for icons. Type in the words after fa-",
			"id" => $prefix."_icon",
			"type" => "text",
			"std" => "",
			// "options" => array('bell','mobile'),
		),

		array( 
			"name" => "Put this notification live?",
			"desc" => "Tick this checkbox to put this notification live.",
			"id" => $prefix."_live",
			"type" => "checkbox",
		),
...
add_action('admin_menu', 'notification_add_boxel');
// Add meta boxel
function notification_add_boxel() {
	global $meta_boxel;
	foreach ( array( 'notification' ) as $page )
	add_meta_box($meta_boxel['id'], $meta_boxel['title'], 'notification_show_boxel', $page, $meta_boxel['context'], 			$meta_boxel['priority']);
}
// Callback function to show fields in meta boxel
function notification_show_boxel() {
...
jQuery(document).ready(function() {
	...
	echo '<input type="hidden" name="notification_meta_boxel_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
...
add_action('save_post', 'notification_save_data');
// Save data from meta boxel
function notification_save_data($post_id) {
	global $meta_boxel;	
	// verify nonce
	if (!wp_verify_nonce($_POST['notification_meta_boxel_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	// check autosave
